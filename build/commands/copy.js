...
import { copyEnvFilesAndExtras, installDependencies, replicateWorkingTreeState, duplicateGitIndex, } from "../utils/worktree.js";
function generateBranchName(currentBranch) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "");
    const base = (currentBranch && currentBranch.length > 0 ? currentBranch : "detached").replace(/[^a-zA-Z0-9._/-]+/g, "-");
    return `copy/${base}-${timestamp}`;
}
export async function copyWorktreeHandler(branchName, options = {}) {
    try {
